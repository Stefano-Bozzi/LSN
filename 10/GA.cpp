...
capitals: sono liste di città ordinate, fatte nel seguente modo: nella posizione 0 (chiamata anche L2) vi è la quantità L2 appunto, caratteristica della lista
...
ESEMPIO DI POIZIONE DI ELEMENTI:	elemento L2 di una lista capitals: capitals[0][0] (oppure capitals[L2][0])
					
					coordinata x della prima città di una lista capitals: capitals[1][x]
					
					coordinata y della seconda città di una lista capitals: capitals[2][y]
					
					elemento L2 della seconda lista capitals di una popolazione Pop: Pop[1][L2][0]
					
					coordinata y della prima città della terza lista capitals di una popolazione Pop: Pop[3][1][y]
//*/
...
//lettura da file
	...
	for (int s=0; s<Ngen; s++){
		...
		//if (rank==0&&s>20)cout<<"\t"<<"\t"<<"\t"<<"done if and NewPop scambio>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;
